...
class WebLurker:
    ...
    def _Extractorworker(self, work_Queue, done_Queue):
        for uc in iter(work_Queue.get, 'STOP'):
            de = DataExtractor(uc.getRawData(), fextractors=self._fExtractors, rextractors=self._rExtractors, cleanExtractorVals=self._cleanExtractorVals)
            de.extract()
            done_Queue.put(de)
        return True

    def _Refinerworker(self, work_Queue, done_Queue):
        for de in iter(work_Queue.get, 'STOP'):
            dr = DataRefiner(de.getExtractedData(), self._refiners)
            dr.refine()
            done_Queue.put(dr)
        return True
